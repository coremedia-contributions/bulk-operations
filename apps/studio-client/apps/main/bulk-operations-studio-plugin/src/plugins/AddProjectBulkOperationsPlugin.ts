...
import ProjectContentControlsToolbar
  from "@coremedia/studio-client.main.control-room-editor-components/project/components/ProjectContentControlsToolbar";
...
class AddProjectBulkOperationsPlugin extends NestedRulesPlugin {
  ...
  constructor(config: Config<AddProjectBulkOperationsPlugin> = null) {
    ...
    super(ConfigUtils.apply(Config(AddProjectBulkOperationsPlugin, {
      rules: [
        Config(ProjectContentControlsToolbar, {
          plugins: [
            Config(AddItemsPlugin, {
              ...
              after: [
                Config(Component, { itemId: ProjectContentControlsToolbar.WITHDRAW_CONTENT_ITEM_ID })
              ]
            }),
          ],
        }),
